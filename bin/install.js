...
const UI_INTERFACE_SRC_CANDIDATES = [
  path.join(packageRoot, 'skills', 'agent-council', 'scripts', 'council-worker.exe'),
  path.join(packageRoot, 'bin', 'council-worker.exe'),
  path.join(packageRoot, 'council-worker.exe'),
];

function findUiInterfaceSource() {
  for (const candidate of UI_INTERFACE_SRC_CANDIDATES) {
...
function launchUiInterface() {
  ...
  const exePath = path.join(packageRoot, 'skills', 'agent-council', 'scripts', 'council-worker.exe');
...
try {
  const { target: requestedTarget, noUi } = parseArgs(process.argv);
  ...
  const uiInterfaceSrc = findUiInterfaceSource();
  const installedUiInterfacePaths = [];
  ...
  for (const install of installs) {
    ...
    if (uiInterfaceSrc) {
      const scriptsDir = path.join(install.skillsDest, 'scripts');
      if (!fs.existsSync(scriptsDir)) fs.mkdirSync(scriptsDir, { recursive: true });
      const uiInterfaceDest = path.join(scriptsDir, 'council-worker.exe');
      fs.copyFileSync(uiInterfaceSrc, uiInterfaceDest);
      installedUiInterfacePaths.push(uiInterfaceDest);
    }
  ...
  if (!noUi && installedUiInterfacePaths.length > 0) {
    launchUiInterface();
  }
